...
public abstract class NearestElevatorStrategy implements ElevatorSelectionStrategy {
    @Override
    public Elevator getOptimalElevator(List<Elevator> elevators, Request request) {
        ...
        for (Elevator elevator : optimalElevators) {
            int distance = Math.abs(request.getDestination() - elevator.getCurrentFloor());
            if (distance < max) {
                max = distance;
